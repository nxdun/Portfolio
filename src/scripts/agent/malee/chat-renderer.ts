...
import type { ProductCard, ProductDetailView, CartView, CartItem, CheckoutDraft, TrackingResult, TrackingTimeline } from "./store";
...
export class ChatRenderer {
  ...
  constructor(private root: HTMLElement, private store: MaleeStore) {
  ...
  private renderProductCarousel(title: string, subtitle: string | undefined, items: ProductCard[]): void {
    ...
    wrapper.className = "w-full my-12 flex flex-col items-center animate-in fade-in slide-in-from-bottom-8 duration-[1200ms] relative";
    ...
    carouselArea.className = "relative w-full max-w-[700px] h-[360px] sm:h-[400px] flex items-center justify-center perspective-[1200px] select-none touch-pan-y z-30";
    ...
    items.forEach((item, i) => {
      ...
      cardWrapper.className = "absolute w-[220px] h-[320px] sm:w-[260px] sm:h-[360px] transition-all duration-[800ms] ease-[cubic-bezier(0.2,0.8,0.2,1)] flex items-center justify-center cursor-pointer group";
      ...
      const btn = cardWrapper.querySelector('.product-add-btn') as HTMLButtonElement;
      if (btn && item.in_stock) {
        btn.onclick = (e) => {
          ...
          btn.classList.remove('bg-white/15', 'hover:bg-white', 'hover:text-black');
          
          cardWrapper.dispatchEvent(new CustomEvent("action:add_to_cart", { bubbles: true, detail: { product_id: item.id, name: item.name, price_lkr: item.price_lkr, image_url: item.image_url }}));
          setTimeout(() => {
            btn.innerHTML = orig;
            btn.classList.remove('bg-green-500', 'border-green-500', 'text-white');
            btn.classList.add('bg-white/15', 'hover:bg-white', 'hover:text-black');
          }, 2000);
        };
      }
    ...
    const btnPrev = carouselArea.querySelector('#nav-prev') as HTMLButtonElement;
    const btnNext = carouselArea.querySelector('#nav-next') as HTMLButtonElement;
    
    btnPrev.onclick = () => { if (activeIndex > 0) { activeIndex--; updateCarousel(); } };
    btnNext.onclick = () => { if (activeIndex < cards.length - 1) { activeIndex++; updateCarousel(); } };
    ...
    items.forEach((_, i) => {
      ...
      dot.onclick = () => { activeIndex = i; updateCarousel(); };
      dots.push(dot);
    ...
    const updateCarousel = () => {
      ...
      cards.forEach((card, i) => {
        ...
        if (diff === 0) {
          overlay.style.background = 'linear-gradient(to top, rgba(0,0,0,0.9), rgba(0,0,0,0.2) 60%, transparent)';
          content.classList.remove('translate-y-4');
          content.style.opacity = '1';
          if (innerImg) { innerImg.style.transform = 'scale(1)'; innerImg.style.opacity = '1'; }
          if (actionBtn) { actionBtn.style.opacity = '1'; actionBtn.style.pointerEvents = 'auto'; }
        } else {
          overlay.style.background = 'linear-gradient(to top, rgba(0,0,0,0.95), rgba(0,0,0,0.7) 60%, rgba(0,0,0,0.4))';
          content.classList.add('translate-y-4');
          content.style.opacity = '0';
          if (innerImg) { innerImg.style.transform = 'scale(1.1)'; innerImg.style.opacity = '0.6'; }
          if (actionBtn) { actionBtn.style.opacity = '0'; actionBtn.style.pointerEvents = 'none'; }
        }
      });
    ...
    carouselArea.addEventListener('mousedown', (e) => { startX = e.pageX; isDragging = true; });
  ...
  private renderProductDetail(item: ProductDetailView): void {
    ...
    card.className = "relative w-full max-w-[340px] sm:max-w-[380px] self-start my-6 group cursor-default select-none animate-in fade-in slide-in-from-bottom-8 duration-1000 z-10";
    ...
    if (btn && item.in_stock) {
      btn.onclick = (e) => {
        ...
        card.dispatchEvent(new CustomEvent("action:add_to_cart", {
          bubbles: true,
          detail: { product_id: item.id, name: item.name, price_lkr: item.price_lkr, image_url: item.image_urls?.[0] }
        }));
        setTimeout(() => {
          btn.innerHTML = orig;
          btn.classList.remove('bg-green-500', 'border-green-500', 'text-white');
  ...
  private renderCategoryGrid(categories: string[]): void {
    const wrapper = document.createElement("div");
    wrapper.className = "w-full max-w-[340px] sm:max-w-[380px] self-start my-6 z-10 animate-in fade-in slide-in-from-bottom-6 duration-1000";
    ...
    categories.forEach((cat, i) => {
      ...
      btn.innerHTML = `
        ...
        <span class="text-sm sm:text-base font-semibold tracking-wide text-white group-hover:text-white transition-colors z-10 drop-shadow-md leading-tight">${cat}</span>
      `;
  ...
  private renderCitySuggestions(cities: string[]): void {
    const wrapper = document.createElement("div");
    wrapper.className = "flex flex-col gap-3 w-full max-w-[340px] sm:max-w-[380px] self-start my-6 z-10 animate-in fade-in slide-in-from-bottom-6 duration-1000";
  ...
  private renderDeliveryQuote(city: string, date: string, rateLkr: number, deliverable: boolean, perishableWarning: boolean, nextAvailable: string | null): void {
    ...
    const glowColor = isError ? 'bg-red-500/20' : (isWarning ? 'bg-amber-500/20' : 'bg-green-500/20');
    const borderColor = isError ? 'border-red-500/30' : (isWarning ? 'border-amber-500/30' : 'border-green-500/30');
    const iconBg = isError ? 'bg-red-500/20' : (isWarning ? 'bg-amber-500/20' : 'bg-green-500/20');
    const iconColor = isError ? 'text-red-500' : (isWarning ? 'text-amber-500' : 'text-green-500');
    const statusText = isError ? 'Unavailable' : (isWarning ? 'Restricted' : 'Available');
  ...
  private renderCheckoutReady(payUrl: string, orderRef: string, expiresMins: number, cartSummary: CartItem[]): void {
    const wrapper = document.createElement("div");
    wrapper.className = "relative w-full max-w-[340px] sm:max-w-[400px] self-start my-6 z-10 animate-in fade-in slide-in-from-bottom-8 duration-1000";
    
    let itemsHtml = "";
    if (cartSummary && cartSummary.length > 0) {
      const total = cartSummary.reduce((acc, curr) => acc + (curr.price_lkr * curr.quantity), 0);
  ...
  private renderTrackingResult(result: TrackingResult): void {
    ...
    let timelineHtml = '<div class="flex flex-col pl-4 relative before:absolute before:left-[21px] before:top-2 before:bottom-4 before:w-px before:bg-gradient-to-b before:from-accent/50 before:to-transparent z-10">';
