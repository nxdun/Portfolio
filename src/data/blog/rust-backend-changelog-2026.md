...
- <a href="https://github.com/nxdun/rust-codebase/pull/11" target="_blank"><code>#11</code></a>[feat(backend)] Add GitHub contributions API endpoint
  ...
  - API controller and routes for `/api/v1/contributions?username=` (only default username permitted).
  - Full `ContributionsResponse` model matching GitHub calendar schema.
  - Env config support for `GITHUB_PAT` and `GITHUB_USERNAME`; cloud-init template updated to pass `GHCR_PAT` as `GITHUB_PAT`.
  - Postman collection tests added for default and specific user endpoints.
