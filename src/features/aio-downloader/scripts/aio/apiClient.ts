...
export type AioDownloadFile = {
  blob: Blob;
  fileName: string;
};

export type AioBrowserDownloadProgress = {
  loadedBytes: number;
...
type AioDownloadOptions = {
  onProgress?: (progress: AioBrowserDownloadProgress) => void;
};

export type AioJobSnapshot = {
  status: string;
...
export type AioStreamProgress = AioJobSnapshot;

export type AioStreamError = {
  errorType: "NETWORK_DOWN" | "BAD_REQUEST" | "UNKNOWN";
  message: string;
  shouldFallbackToPolling: boolean;
};

export type AioStreamSubscription = {
  close: () => void;
};

type AioStreamCallbacks = {
  onProgress: (progress: AioStreamProgress) => void;
  onDone: () => void;
  onError: (error: AioStreamError) => void;
};
...
function toSnapshot(job: Record<string, unknown>): AioJobSnapshot {
  return {
    status: normalizeAioJobStatus(readStatus(job)),
...
export class AioApiClient extends CoreApiClient {
  private async fetchJob(
    jobId: string,
    signal?: AbortSignal
  ): Promise<ApiResult<Record<string, unknown>>> {
    const response = await this.getJson(
      `${AIO_JOB_PATH}/${encodeURIComponent(jobId)}`,
      signal
    );
  ...
  openJobStream(
    jobId: string,
    callbacks: AioStreamCallbacks
  ): ApiResult<AioStreamSubscription> {
    const target = `${AIO_JOB_PATH}/${encodeURIComponent(jobId)}${AIO_STREAM_SUFFIX}`;
  ...
  async getSites(signal?: AbortSignal): Promise<ApiResult<string[]>> {
    const response = await this.getJson(AIO_SITES_PATH, signal);
  ...
  async checkJobStatus(
    jobId: string,
    signal?: AbortSignal
  ): Promise<ApiResult<AioJobState>> {
    const jobResult = await this.fetchJob(jobId, signal);
    if (!jobResult.ok) {
      return jobResult;
    }

    const status = normalizeAioJobStatus(readStatus(jobResult.data));
    const terminal = toAioTerminalState(status);
    if (terminal === "fail") {
  ...
  async getJobSnapshot(
    jobId: string,
    signal?: AbortSignal
  ): Promise<ApiResult<AioJobSnapshot>> {
    const jobResult = await this.fetchJob(jobId, signal);
    if (!jobResult.ok) {
      return jobResult;
    }
  ...
  async downloadFile(
    jobId: string,
    signal?: AbortSignal,
    options?: AioDownloadOptions
  ): Promise<ApiResult<AioDownloadFile>> {
    try {
      const response = await fetch(
        this.resolveUrl(`${AIO_DOWNLOAD_PATH}/${encodeURIComponent(jobId)}`),
        {
          method: "GET",
          signal,
        }
