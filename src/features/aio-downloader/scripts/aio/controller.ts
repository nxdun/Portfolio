import { createToolResponseDock } from "@/scripts/tools/ui/responseDock";
import { readClipboardText } from "@/scripts/tools/clipboard";
import type { ToolTeardown, AioToolOptions } from "@/scripts/tools/types";
import { validateAioActionInput } from "./validation";
import type { AioApiClient, AioBrowserDownloadProgress } from "./apiClient";
import { CaptchaManager } from "@/utils/captchaManager";
import type { ApiErrorType, ApiResult } from "@/utils/CoreApiClient";
import type { AioDomRefs } from "./dom";
import { AioUiController } from "./uiController";
import type { CaptchaDialogController } from "@/scripts/tools/ui/captchaDialog";
import { AioSitesController } from "./sitesController";
import { AioJobMonitor, type StreamMonitorResult } from "./jobMonitor";
...
function formatBrowserDownloadMeta(
  progress: AioBrowserDownloadProgress
): string {
...
export class AioToolController {
  ...
  constructor(config: AioControllerConfig) {
    ...
    this.sitesController = new AioSitesController(this.refs, this.apiClient);

    this.jobMonitor = new AioJobMonitor(
      this.apiClient,
      this.ui,
  ...
  private async handleSubmitClick(): Promise<void> {
    ...
    const urlValidation = validateAioActionInput("enqueue", {
  ...
  private async handleVerifyCaptchaClick(): Promise<void> {
    ...
    const captchaValidation = validateAioActionInput("verify-captcha", {
      backendUrl: this.backendUrl,
