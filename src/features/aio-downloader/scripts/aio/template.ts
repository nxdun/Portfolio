export const AIO_TOOL_TEMPLATE = `
  <section class="grid gap-6">
    <div class="grid gap-2 relative group/input">
      <div class="flex items-center justify-between">
        <label class="text-sm font-semibold flex items-center gap-2" for="aio-url-input">
          Video URL
        </label>
      ...
      />
      <div class="flex items-center justify-between">
        <p class="text-xs opacity-75">Paste a video URL from one of <button type="button" id="aio-view-sites" class="font-bold text-accent hover:underline outline-none">2000+ supported sites</button>.</p>
      </div>
    </div>
    ...
    <dialog id="aio-sites-dialog" class="m-auto h-[min(82vh,760px)] w-[94vw] max-w-5xl overflow-hidden rounded-2xl border border-border/60 bg-background/95 p-0 text-foreground shadow-2xl backdrop:bg-background/80 backdrop:backdrop-blur-sm">
      <div class="flex h-full flex-col">
        <header class="border-b border-border/50 px-5 py-4 sm:px-6">
          <div class="flex items-start justify-between gap-4">
            <div class="grid gap-1">
              <h3 class="text-lg font-bold leading-none">Supported Sites (<span id="aio-sites-count">0</span>+)</h3>
              <p class="text-xs text-muted-foreground">Search across providers and verify your URL host quickly.</p>
            </div>
            <button id="aio-sites-close" type="button" class="rounded-md p-1 text-muted-foreground transition-colors hover:bg-muted/40 hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent" aria-label="Close supported sites dialog">
        ...
        <div class="min-h-0 flex-1 px-5 py-4 sm:px-6">
          <div id="aio-sites-list" class="grid h-full min-h-56 grid-cols-1 content-start gap-2 overflow-y-auto rounded-xl border border-border/50 bg-muted/10 p-2 text-sm sm:p-3 md:grid-cols-2">
